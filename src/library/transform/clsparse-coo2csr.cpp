                   ...
                   const clsparseControl control)
{
    ...
    clsparse::vector<size_t> csr_row_offsets (control, csr->rowOffsets, csr->num_rows + 1);
    clsparse::vector<size_t> csr_col_indices (control, csr->colIndices, csr->num_nonzeros);
    clsparse::vector<cl_float> csr_values (control, csr->values, csr->num_nonzeros);

    clsparse::vector<size_t> coo_row_indices (control, coo->rowIndices, coo->num_nonzeros);
    clsparse::vector<size_t> coo_col_indices (control, coo->colIndices, coo->num_nonzeros);
